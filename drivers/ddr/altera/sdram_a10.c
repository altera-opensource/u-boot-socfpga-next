...
#define EMIF_RST_MASK		(1 << 31) /* fpga_mgr_fpgamgrregs.gpo.31 */
#define OCT_CAL_REQ_MASK	(1 << 30) /* fpga_mgr_fpgamgrregs.gpo.30 */
#define OCT_CAL_ACK_MASK	(1 << 31) /* fpga_mgr_fpgamgrregs.gpi.31 */

#define NIOS_OCT_DONE_MASK	(1 << 7)
...
static const struct socfpga_fpga_manager *socfpga_fpga_mgr_base =
		(void *)SOCFPGA_FPGAMGRREGS_ADDRESS;
static const struct socfpga_reset_manager *socfpga_reset_mgr_base =
...
int ddr_calibration(void)
{
	...
	clrbits_le32(&socfpga_fpga_mgr_base->gpo, EMIF_RST_MASK);
	mdelay(10);

	/* Step 2 - Clearing registers to EMIF core */
	writel(0, &socfpga_reset_mgr_base->hmcgpout);	/*Clear the HPS->NIOS COM reg.*/

	/* Step 3 - Clearing registers to OCT core */
	clrbits_le32(&socfpga_fpga_mgr_base->gpo, OCT_CAL_REQ_MASK);
	mdelay(5);

	/* Step 4 - Taking EMIF out of reset */
	setbits_le32(&socfpga_fpga_mgr_base->gpo, EMIF_RST_MASK);
	mdelay(10);

	/* Step 5 - Waiting for OCT circuitry to come out of reset */
	timeout = 100;
	while (((readl(&socfpga_fpga_mgr_base->gpin) & OCT_CAL_ACK_MASK) == 0) &&
		timeout > 0) {
		mdelay(1);
		timeout--;
	}
	...
	setbits_le32(&socfpga_reset_mgr_base->hmcgpout, NIOS_OCT_DONE_MASK);

	/* Step 7 - Waiting for EMIF request */
	timeout = 200;
	while (((readl(&socfpga_reset_mgr_base->hmcgpin) & NIOS_OCT_ACK_MASK) == 0) &&
		timeout > 0) {
		mdelay(1);
		timeout--;
	}
	...
	clrbits_le32(&socfpga_reset_mgr_base->hmcgpout, NIOS_OCT_DONE_MASK);
	...
	timeout = 10;
	while (((readl(&socfpga_reset_mgr_base->hmcgpin) & OCT_CAL_ACK_MASK) == 0) &&
		timeout > 0) {
		udelay(100);
		timeout--;
	}
...
void sdram_mmr_init(void)
{
	...
	u32 ddrioctl;
	u32 cfg_addr_order, cfg_bank_addr_width, cfg_bank_group_addr_width;
	u32 cfg_col_addr_width, cfg_row_addr_width, cfg_rd_to_pch;
	u32 cfg_pch_to_valid, cfg_act_to_rdwr, cfg_ctrl_burst_len;
	u32 cfg_rd_to_wr, cfg_act_to_act, cfg_wr_to_rd;
	u32 cfg_act_to_act_db, cfg_4_act_to_act, cfg_rd_to_rd_dc;
	u32 cfg_rd_to_wr_dc, cfg_wr_to_rd_dc;
